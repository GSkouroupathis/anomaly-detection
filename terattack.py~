...
class TerMimicry(Attack):
	...
	def attack(self, sensor_id, goal, starting_index, sensor_set):
		...
		final_stitch = self.stitch(z[lowest_temp_index], goal, sensor_set, (len(z) - lowest_temp_index))
	...
	def stitch(self, start_temp, end_temp, sensor_set, length):
		...
		for sensor in sensor_set:
			...
			if len(z) > 0:
				...
				while start_index < len(z):
					(current_replay, last_i) = self.find_replay_values(sensor, z, first_order, S_N_array, start_index, len(z), current_start_temp, end_temp)
