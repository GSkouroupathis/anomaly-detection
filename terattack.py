...
class TerMimicry(Attack):
	...
	def attack(self, sensor_z, goal, starting_index, sensor_set_z):
		...
		final_stitch = self.stitch(z[lowest_temp_index], goal, sensor_set_z, (len(z) - lowest_temp_index))
		...
		if (max(final_attacked_values) < goal):
			...
			final_attacked_values = self.add_sin(final_attacked_values, DT, lowest_temp_index, len(final_attacked_values))
		
		while abs((len(z)-lowest_temp_index)*1.0 / (  len(final_attacked_values) - lowest_temp_index) ) >= self.STRETCH_RATIO_THRESHOLD:
			final_attacked_values[lowest_temp_index:] = self.stretch(final_attacked_values[lowest_temp_index:])
		
		return (final_attacked_values, lowest_temp_index)
	
	# takes data from a set and stitch past measurements together
	def stitch(self, start_temp, end_temp, sensor_set_z, length):
		stitched_value_index = 0
		final_stitch = []
		current_start_temp = start_temp
	
		for z in sensor_set_z:
			if len(z) > 0:
				z_array = np.array(z)
				...
				while start_index < len(z):
					(current_replay, last_i) = self.find_replay_values(None, z, first_order, S_N_array, start_index, len(z), current_start_temp, end_temp)
	...
	def find_replay_values(self, sensor_id, z, first_order, S_N_array, start_window, end_window, start_temp, end_temp):
		if z == []:
			return []
		...
		for i in range(start_window, end_window):
			if (abs(z[i] - start_temp) <= self.TEMPERATURE_DIFF_THRESHOLD and start_index == -1):
				start_index = i
			
			if (start_index != -1):
				if (abs(z[i] - end_temp) <= self.TEMPERATURE_DIFF_THRESHOLD):
					end_index = i
					break
				
			if (abs(first_order[i]) >= self.GRADIENT_THRESHOLD):
				break
			
			if (abs(S_N_array[min((len(first_order)-1) , (i+10))]) >= self.SN_THRESHOLD):
				break
	...
	def add_sin(self, z, DT, starting_index, end_index):
		sin_z = z[:]
